...
int main() {
    ...
    case019_test::Expect(ranked.size() == 5, "ranked result size changed unexpectedly");
    case019_test::Expect(case019_test::has_reason_summary<nitr::case019::RankedItem>::value,
                         "RankedItem must expose reason_summary");

    if constexpr (!case019_test::has_reason_summary<nitr::case019::RankedItem>::value) {
        return 1;
    } else {
        ...
        if constexpr (!case019_test::has_summary_final_score<Summary>::value ||
            ...
            case019_test::Expect(ranked[0].item.id == 10, "top match should remain first");
            case019_test::Expect(ranked[0].reason_summary.final_score == ranked[0].final_score,
            ...
            case019_test::Expect(ranked[0].reason_summary.strongest_positive == nitr::case019::ReasonCode::kCategoryMatch,
                                 "top match strongest positive factor is incorrect");
            case019_test::Expect(ranked[0].reason_summary.strongest_negative == nitr::case019::ReasonCode::kNone,
                                 "top match should not have a strongest negative factor");

            case019_test::Expect(ranked[3].item.id == 12, "wrong category item should stay in fourth place");
            case019_test::Expect(ranked[3].reason_summary.final_score == 86,
                                 "wrong category final score is incorrect");
            case019_test::Expect(ranked[3].reason_summary.strongest_positive == nitr::case019::ReasonCode::kFreshnessBoost,
                                 "wrong category strongest positive factor is incorrect");
            case019_test::Expect(ranked[3].reason_summary.strongest_negative == nitr::case019::ReasonCode::kCategoryMismatch,
                                 "wrong category strongest negative factor is incorrect");

            case019_test::Expect(ranked[4].item.id == 11, "stale match should remain last among eligible items");
            case019_test::Expect(ranked[4].reason_summary.final_score == 80,
                                 "stale match final score is incorrect");
            case019_test::Expect(ranked[4].reason_summary.strongest_positive == nitr::case019::ReasonCode::kCategoryMatch,
                                 "stale match strongest positive factor is incorrect");
            case019_test::Expect(ranked[4].reason_summary.strongest_negative == nitr::case019::ReasonCode::kStalePenalty,
