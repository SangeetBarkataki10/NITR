...
int main() {
    ...
    case019_test::Expect(case019_test::has_inspect_method<nitr::case019::Ranker>::value,
                         "Ranker must provide Inspect(items, id)");
    case019_test::Expect(case019_test::has_compare_method<nitr::case019::Ranker>::value,
                         "Ranker must provide Compare(items, winner_id, loser_id)");

    if constexpr (!case019_test::has_inspect_method<nitr::case019::Ranker>::value ||
        ...
        case019_test::Expect(case019_test::has_inspection_positive<Inspection>::value,
                             "inspection result must include strongest_positive");
        case019_test::Expect(case019_test::has_inspection_negative<Inspection>::value,
        ...
        case019_test::Expect(case019_test::has_tie_break_against_id<Inspection>::value,
        ...
        if constexpr (!case019_test::has_status_field<Inspection>::value ||
            ...
            case019_test::Expect(blocked.status == nitr::case019::InspectionStatus::kBlocked,
                                 "blocked item inspection status is incorrect");
            case019_test::Expect(blocked.strongest_positive == nitr::case019::ReasonCode::kNone,
                                 "blocked item should not report a positive factor");
            case019_test::Expect(blocked.strongest_negative == nitr::case019::ReasonCode::kNone,
            ...
            case019_test::Expect(tie_loser.status == nitr::case019::InspectionStatus::kRanked,
            ...
            case019_test::Expect(tie_loser.tie_break_against_id == 14,
                                 "tie loser should identify the winner of the tie-break");
            case019_test::Expect(tie_loser.strongest_positive == nitr::case019::ReasonCode::kCategoryMatch,
                                 "tie loser strongest positive factor is incorrect");
            case019_test::Expect(tie_loser.strongest_negative == nitr::case019::ReasonCode::kNone,
                                 "tie loser should not report a negative factor");

            const auto missing = ranker.Inspect(items, 404);
            case019_test::Expect(missing.status == nitr::case019::InspectionStatus::kNotFound,
            ...
            case019_test::Expect(case019_test::has_decided_by_tie_break<Comparison>::value,
            ...
            if constexpr (!case019_test::has_status_field<Comparison>::value ||
                ...
                case019_test::Expect(comparison.status == nitr::case019::ComparisonStatus::kOk,
                ...
                case019_test::Expect(comparison.decisive_reason == nitr::case019::ReasonCode::kBaseScoreTieBreak,
                                     "comparison decisive reason is incorrect");

                const auto blocked_comparison = ranker.Compare(items, 10, 13);
                case019_test::Expect(blocked_comparison.status == nitr::case019::ComparisonStatus::kNotApplicable,
