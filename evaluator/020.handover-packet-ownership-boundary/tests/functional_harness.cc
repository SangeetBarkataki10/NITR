...
void PrintSingleOutputCase(const std::string& scenario_name,
    ...
    const std::string preview = nitr::case020::BuildHandoverPacketPreview(tracker);
    ...
    std::cout << "\"last_row_in_progress\":"
              << ((packet.rows.empty() || !packet.rows.back().from_in_progress_tote) ? "false" : "true");
    std::cout << "}";
}

void PrintSequenceCase(const std::string& output_path_a, const std::string& output_path_b) {
    nitr::case020::ShiftTracker tracker = BuildInProgressTracker();

    const std::string preview_first = nitr::case020::BuildHandoverPacketPreview(tracker);
    ...
    const std::string preview_second = nitr::case020::BuildHandoverPacketPreview(tracker);
    ...
    std::cout << "\"package_count_second\":" << packet_second.summary.package_count;
    std::cout << "}";
}
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "usage: functional_harness <scenario> <output_path_a> [output_path_b]\n";
        return 1;
    }
