...
void TestBaselineQueryBehavior() {
  ...
  Expect(result.value == "one", "baseline: found value should be from active snapshot");
...
void TestActivateReplaceBehavior() {
  ...
  Expect(result.value == "beta", "replace: query should not return stale v1 value");
...
void TestResetAndNoActiveBehavior() {
  ...
  Expect(result.value.empty(), "reset: no-active result should not return value");
...
void TestUnknownActivationDoesNotReplaceActive() {
  ...
  Expect(store.ActivateSnapshot("v1"), "unknown: activating known snapshot should succeed");

  const bool activated_unknown = store.ActivateSnapshot("missing-version");
  Expect(!activated_unknown, "unknown: activating unknown snapshot should fail");

  const auto result = query.Lookup("city");
  ExpectStatus(result, nitr::case017::QueryStatus::kFound,
               "unknown: failed activation must keep previous active snapshot");
  Expect(result.value == "rome", "unknown: active snapshot should remain unchanged");
}
...
void TestDuplicateRegistrationRejected() {
  ...
  Expect(result.value == "first", "dup: duplicate registration must not replace payload");
}
...
void TestRepeatedTransitionsAndIsolation() {
  ...
  ExpectStatus(result, nitr::case017::QueryStatus::kFound, "seq: v1 lookup should be found");
  ...
  Expect(result.value == "B", "seq: after v1->v2, query must not return v1 value");

  Expect(store.ActivateSnapshot("v3"), "seq: activate v3 should succeed");
  result = query.Lookup("shared");
  Expect(result.value == "C", "seq: after v2->v3, query must not return older value");
  ...
  Expect(store.ActivateSnapshot("v2"), "seq: activate v2 after reset should succeed");
  result = query.Lookup("shared");
  Expect(result.value == "B", "seq: reset->reactivate should use newly active snapshot");
}
