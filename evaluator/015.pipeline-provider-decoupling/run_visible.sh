...
BUILD_DIR="${BUILD_DIR:-${CASE_ROOT}/build-visible}"
...
main() {
  ...
  run_cmd "${CMAKE_BIN}" -S . -B "${BUILD_DIR}" -DCASE015_ENABLE_VISIBLE_CHECKS=ON -DCASE015_ENABLE_HIDDEN_EVALUATOR=OFF || exit 10
  run_cmd "${CMAKE_BIN}" --build "${BUILD_DIR}" || exit 11
  run_cmd "${CTEST_BIN}" --test-dir "${BUILD_DIR}" --output-on-failure || exit 12
  run_check_allowed_file_touches || exit 13

  log 'Visible evaluator passed.'
}
